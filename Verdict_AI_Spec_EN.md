...
### Version 13.0 | Final Architecture Specification (8-Agent Agentic Pipeline)
...
## Table of Contents
1. Problem & Mission
2. System Overview: Why 8 Agents, Not 1
3. The 8-Agent Architecture (Detailed)
   - Agent 1: Mapper (v1.2)
   - Agent 2: Dimension QA Agent (v1.1) *(new)*
   - Agent 3: Strategist (v1.1)
   - Agent 4: Inquisitor (v1.3)
   - Agent 5: Sentinel *(pure code, no AI)*
   - Agent 6: Hybrid Authenticity & Language Style Analyzer (v2.0) *(new — conditional)*
   - Agent 7: Auditor (v1.2) + Schema Validator *(pure code)*
   - Agent 8: Integration Coordinator (v1.0) *(Computer Use / API Scheduler)*
...
## Chapter 2: System Overview — Why 8 Agents, Not 1
...
| 6 | **Hybrid Authenticity Engine** | AI (GLM-4) | **Conditional only** | Detect authenticity anomalies when Sentinel Stage 2 alert fires |
| 7 | **Auditor + Validator** | AI (GLM-4) + **Pure Code** | Once per session end | Score transcript; code validates output completeness |
| 8 | **Integration Coordinator** | AI (GLM-4) | **Post-Interview (HR Trigger)** | Operates local APIs (Gmail, Calendar, Zoom) to auto-schedule interviews. Acts as a Computer Use Agent. |

**Total AI Prompts: 7** (Mapper, Dimension QA, Strategist, Inquisitor, Language Style Analyzer, Auditor, Integration Coordinator)
...
## Chapter 3: The 8-Agent Architecture (Detailed)
...
### Agent 6: Hybrid Authenticity & Language Style Analyzer (v2.0) *(New — Conditional)*
**Role:** Forensic text analyst. Only activated when Sentinel fires a Stage 2 alert. Compares the candidate's early-session and late-session writing style across 7 signals to detect within-session authenticity anomalies. Crucially, this agent **does not claim to detect AI authorship with certainty**; instead, it flags structural deviations that strongly correlate with external assistance when combined with behavioural delays.

**Triggered:** Conditionally — only when `focus_loss_events > 3 AND paste_events > 1`.

**Inspired by:** Academic anti-plagiarism research (Jisc, Turnitin methodologies) on how externally-assisted text differs from spontaneous human writing. Known signals include: scope drift (over-answering a narrow question), loss of sentence variance (predictability/burstiness), sudden length spikes, appearance of formal discourse markers, and collapse of natural code-switching patterns.

**7 Style Signals (Tiered):**
| Tier | Signal | What it detects | Max Penalty |
|---|---|---|---|
| **Tier 1** | **Scope Drift / Over-Explanation** | Answers expand into generic, comprehensive, or bulleted "best practices" far beyond the narrow question asked. | -20 |
| **Tier 1** | **Response Length Shift vs Complexity** | Late-half answers ≥2.5× longer than early-half with no question complexity increase, usually after a long delay. | -20 |
| **Tier 1** | **Language Register Jump** | Candidate using Manglish suddenly switches to perfect formal English (complete: ≥80% of late turns). Abrupt and one-directional. | -20 |
| **Tier 1** | **Personal Detail Density Drop** | Early half has specific names/numbers/dates; late half becomes generic and universal (loss of personal texture). | -20 |
| **Tier 2** | **Discourse Marker Emergence** | Structured formal transitions ("Firstly,", "In conclusion,") appear in late half, absent in early half. | -10 |
| **Tier 2** | **Sentence Uniformity & Repetition** | Late half consists of evenly structured sentences of similar lengths, repetitive phrasing, and low variance (low burstiness). | -10 |
| **Tier 2** | **Colloquial Marker Retention** | "lah", "lor", "kan" present abundantly early, completely absent late (Malaysia-specific signal). | -10 |

**Scoring:**
```
style_consistency_score = 100 − (sum of all signal penalties)
```
- **72–100:** Consistent style — clean
- **44–71:** Minor/Moderate variation — flag to Auditor
- **<44:** Significant anomaly → `PASS_TO_AUDITOR_STRONG_FLAG`

**Defense Mechanism:** By relying on *within-session stylistic discontinuity* combined with *behavioural metadata* (focus loss), TalentBridge AI sidesteps the high false-positive rates of traditional static "AI detectors". We detect the *act of cheating during the session*, not the mathematical signature of the final text alone.

**Output passed to Auditor:**
```json
{
  "style_consistency_score": 30,
  "anomaly_detected": true,
  "primary_anomaly_type": "Complete register switch at session midpoint combined with severe scope drift and loss of personal texture...",
  "recommendation": "PASS_TO_AUDITOR_STRONG_FLAG"
}
```

**Version:** v2.0 (Hybrid Authenticity Engine) | **Prompt Score:** 97/100
...
### Agent 8: The Integration Coordinator (v1.0) *(Computer Use Agent)*
**Role:** The Executive Assistant. Bridging the gap between a successful evaluation and the real-world hiring process, this agent operates external computer applications (via API) on behalf of the HR professional.

**Triggered:** Only when HR reviews a `GREEN` (or exceptionally strong `AMBER`) Verdict Card and clicks "Proceed to Final Interview".

**The Agentic Workflow:**
1. **Input Collection**: HR provides 4 available timeslots in their local timezone.
2. **Gmail API Operation**: Agent drafts and sends a highly personalized, warm email to the candidate sharing the good news and proposing the timeslots.
3. **Wait & Parse**: Agent monitors the inbox for the candidate's reply, parses natural language to extract the chosen time.
4. **Google Calendar API Operation**: Agent creates a calendar invite, adding both HR and Candidate.
5. **Zoom API Operation**: Agent generates a unique Zoom meeting link and injects it into the Calendar invite.
6. **Dashboard Sync**: Agent updates the HR Dashboard, changing the candidate's state from `Pending Review` to `Interview Scheduled`.

**Why this is revolutionary:** This moves TalentBridge from a passive "Assessment Tool" to an active **Agentic Workflow Platform**. It demonstrates the future of "Computer Use" AIs, where agents execute multi-step external actions autonomously based on a single human decision.

**Version:** v1.0 | **Prompt Score:** 94/100
...
|---|---|---|---|
| 🟢 **Green** | Average dimension score ≥ 75, no more than 1 dimension < 50 | Verified strengths + **Next Steps:** "Your profile is prioritized. If approved, our Integration Agent will automatically email you to schedule an interview." | Full Verdict Card + Green badge + AI summary |
| 🟡 **Amber** | Average ≥ 55, at least 2 dimensions ≥ 75, gap is a specific learnable hard skill | Verified strengths + **Next Steps:** AI simulates web search to curate a 3-week custom growth plan + "Re-apply after completion" | Full Verdict Card + Amber badge + upskill plan |
| 🔴 **Red** | Average < 55 OR all dimensions < 60 with zero personal texture | Career Guidance + **Next Steps:** "AI has redirected your profile to matching roles in our network." **No colour label.** | Full Verdict Card + Red badge + human review flag if applicable |
...
6. **Follow-up / End-to-End Next Steps:**
   - **Green:** "Your verified profile has been prioritized. If the employer chooses to proceed, our **Integration Agent** will automatically email you with available interview timeslots. Expect an update within 48 hours."
   - **Amber:** "Let our AI curate a personalized learning path from the web." (Candidate clicks button → sees AI generation terminal) → "Complete this 3-week path and click here to re-apply for this role."
   - **Red:** "Our AI has redirected your profile to matching roles in our network based on your verified strengths. You will be automatically notified if an employer wishes to connect."
